...
#include "FNX_LineShape.h"
#include "FNX_Actor.h"




FNX_LineShape::FNX_LineShape(FNX_Actor *actor, const FNX_LineShapeDesc &desc)
...
bool FNX_LineShape::IsTouchingPoint(const FNX_Vector &point) {
...
inline FNX_Vector FNX_LineShape::GetSize(){
	return Extents;
}

void FNX_LineShape::Update() {
...
int FNX_LineShape::GetNumEdges() {
	return 2;
}

FNX_Vector FNX_LineShape::GetEdgeDir(int i) {
	return Axis[i];
}

void FNX_LineShape::GetInterval_CenExt(const FNX_Vector &axis, float &center, float &extents) {
	center = Center | axis;
	extents = abs(Axis[0] | axis) * Extents.x;
	extents += abs(Axis[1] | axis) * Extents.y;
}

int FNX_LineShape::GetSupportVertices(const FNX_Vector &axis, FNX_Vector *vertices) {
...
bool FNX_LineShape::IsTouchingPoint(const FNX_Vector &point) {
	...
	FNX_Vector local = inv_world * point;
	if (local.x < -Extents.x || local.x > Extents.x) return false;
	if (local.y < -1 || local.y > 1) return false;
	return true;
}
...
int FNX_LineShape::GetNumEdges() {//changed from box
	return 2;
}

FNX_Vector FNX_LineShape::GetEdgeDir(int i) {	//changed from box
	return Axis[i];
}
