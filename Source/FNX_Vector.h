...
class FNX_Vector {
 ...
 public:
  ...
  // Other operators
  float operator|(const FNX_Vector &other) const { return x*other.x + y*other.y; }///dot product
...
FNX_Vector FNX_TranslatePosOffScreenLHToLH(HWND hWnd,float x,float y);///works only with current window
FNX_Vector FNX_TranslatePosLHToOffScreenLH(HWND hWnd,float x,float y);///works only with current window